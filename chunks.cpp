...
sf::Vector2f GridToWorld(float row, float col) {
  return {(col - row) * (tileWidth / 2.0f), (col + row) * (tileHeight / 4.0f)};
}
...
public:
  ...
  void setCoordinates(sf::Vector2i coord) { chunkCoordinates = coord; }
};
...
public:
  ...
  void RenderTile(sf::Vector2i tilePos, int row, int col,
    ...
    currentTile.setPosition(std::round(isoCoord.x + worldPos.x),
                            std::round(isoCoord.y + worldPos.y));
  }
